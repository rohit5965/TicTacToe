...
public class TicTacToeGame extends View {
    ...
    public TicTacToeGame(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        game = new GameLogic();
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs ,R.styleable.TicTacToeGame,0,0);

        try {
            gameColor = a.getInteger(R.styleable.TicTacToeGame_gameColor,0);
            XColor = a.getInteger(R.styleable.TicTacToeGame_XColor,0);
            OColor = a.getInteger(R.styleable.TicTacToeGame_OColor,0);
            winningLineColor = a.getInteger(R.styleable.TicTacToeGame_winningLineColor,0);
        }
        finally {
            a.recycle();
        }
